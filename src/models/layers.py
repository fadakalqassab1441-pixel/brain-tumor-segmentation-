...
class SELayer3D(nn.Module):
    ...
    Channel-only attention: global-average-pools every channel down to a
    single number, learns a per-channel gate through a small bottleneck
    MLP (fc -> ReLU -> fc -> sigmoid), and rescales the input by that gate.
    Unlike CBAM above, there is no spatial-attention branch and no max-pool
    branch — this is intentionally the plain SE block, used in the decoder.
    """
...
class UBlockSE(nn.Sequential):
    """Decoder UBlock followed by a channel-wise Squeeze-and-Excitation gate.

    Same two ConvBnRelu layers as UBlock (so the skip connection + upsampled
    features are merged exactly as before), then SE reweights the resulting
    channels before they are handed to the next (coarser -> finer) stage or
    to the output conv.
    """

    def __init__(self, inplanes, midplanes, outplanes, norm_layer, dilation=(1, 1), dropout=0, se_reduction=8):
        super(UBlockSE, self).__init__(
            OrderedDict(
                [
                    ('UBlock', UBlock(inplanes, midplanes, outplanes, norm_layer, dilation=dilation, dropout=dropout)),
                    ('SE', SELayer3D(outplanes, reduction_ratio=se_reduction)),
                ])
        )
